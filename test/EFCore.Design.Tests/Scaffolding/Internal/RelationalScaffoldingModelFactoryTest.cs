...
namespace Microsoft.EntityFrameworkCore.Internal
{
    public class RelationalDatabaseModelFactoryTest
    {
        ...
        static RelationalDatabaseModelFactoryTest()
        {
            ...
            IdColumn = new DatabaseColumn(Table, "Id") { StoreType = "int" };
            ...
            var info = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "Jobs")
                    {
                        ...
                        {
                            IdColumn,
                            new DatabaseColumn(Table, "occupation")
                            {
                                StoreType = "nvarchar(max)",
                                DefaultValueSql = "\"dev\""
                            },
                            new DatabaseColumn(Table, "salary")
                            {
                                StoreType = "int",
                                IsNullable = true
                            },
                            new DatabaseColumn(Table, "modified")
                            {
                                StoreType = "nvarchar(max)",
                                IsNullable = false,
                                ValueGenerated = ValueGenerated.OnAddOrUpdate
                            },
                            new DatabaseColumn(Table, "created")
                            {
                                StoreType = "nvarchar(max)",
                                ValueGenerated = ValueGenerated.OnAdd
                            },
                            new DatabaseColumn(Table, "current")
                            {
                                StoreType = "nvarchar(max)",
            ...
            var info = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "NaturalProducts")
                    {
                        ...
                        {
                            IdColumn,
                            new DatabaseColumn(Table, "ProductSKU") { StoreType = "nvarchar(max)" },
                            new DatabaseColumn(Table, "supplierID") { StoreType = "nvarchar(max)" },
                            new DatabaseColumn(Table, "Vendor_Discount") { StoreType = "nvarchar(max)" }
                        },
                        PrimaryKey = IdPrimaryKey
                    }
                }
        ...
        {
            var column = new DatabaseColumn(Table, "Col") { StoreType = StoreType };
        ...
        {
            var col1 = new DatabaseColumn(Table, "Col1") { StoreType = "nvarchar(max)" };
            var info = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "A")
                    {
                        ...
                        {
                            col1,
                            new DatabaseColumn(Table, "Col2") { StoreType = "nvarchar(max)" },
                            new DatabaseColumn(Table, "Col3") { StoreType = "nvarchar(max)" }
                        },
        ...
        public void Unmappable_column_type(string StoreType)
        {
            ...
            info.Tables.First()
                .Columns.Add(
                    new DatabaseColumn(info.Tables.First(), "Coli")
                    {
                        StoreType = StoreType
                    });
...
#pragma warning restore xUnit1026 // Theory methods should use all of their parameters

        {
            ...
            foreach (var column in keyProps.Select(
                k => new DatabaseColumn(Table, k) { StoreType = "int" }))
            {
        ...
        {
            var myColumn = new DatabaseColumn(Table, "MyColumn") { StoreType = "int" };
        ...
        {
            var c1 = new DatabaseColumn(Table, "C1") { StoreType = "int" };
            var table = new DatabaseTable(Database, "T")
            {
                Columns =
                {
                    c1,
                    new DatabaseColumn(Table, "C2") { StoreType = "int" },
                    new DatabaseColumn(Table, "C3") { StoreType = "int" }
                },
        ...
        public void Foreign_key()
        {
            ...
            var childrenTable = new DatabaseTable(Database, "Children")
            {
                ...
                {
                    IdColumn,
                    new DatabaseColumn(Table, "ParentId")
                    {
                        StoreType = "int",
                        IsNullable = true
                    }
        ...
        {
            var keyColumn = new DatabaseColumn(Table, "Key")
            {
                StoreType = "int",
                IsNullable = false
            };
        ...
        {
            var ida = new DatabaseColumn(Table, "Id_A") { StoreType = "int" };
            var idb = new DatabaseColumn(Table, "Id_B") { StoreType = "int" };
            var parentTable = new DatabaseTable(Database, "Parent")
            {
                Columns = { ida, idb },
            ...
            var childrenTable = new DatabaseTable(Database, "Children")
            {
                ...
                {
                    IdColumn,
                    new DatabaseColumn(Table, "ParentId_A") { StoreType = "int" },
                    new DatabaseColumn(Table, "ParentId_B") { StoreType = "int" }
                },
                PrimaryKey = IdPrimaryKey
            };
            childrenTable.ForeignKeys.Add(
        ...
        public void It_logs_warning_for_bad_foreign_key()
        {
            var parentTable = new DatabaseTable(Database, "Parent")
            {
                Columns = { IdColumn, new DatabaseColumn(Table, "NotPkId") { StoreType = "int" } },
                PrimaryKey = IdPrimaryKey
            };
            var childrenTable = new DatabaseTable(Database, "Children")
            {
                Columns = { IdColumn, new DatabaseColumn(Table, "ParentId") { StoreType = "int" } },
                PrimaryKey = IdPrimaryKey
            };
            childrenTable.ForeignKeys.Add(
                new DatabaseForeignKey(childrenTable, "FK_Foo", parentTable)
        ...
        public void Unique_nullable_index_unused_by_foreign_key()
        {
            var table = new DatabaseTable(Database, "Friends")
            {
                ...
                {
                    IdColumn,
                    new DatabaseColumn(Table, "BuddyId")
                    {
                        StoreType = "int",
                        IsNullable = true
                    }
            ...
            var info = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "E F")
                    {
                        ...
                        {
                            IdColumn,
                            new DatabaseColumn(Table, "San itized") { StoreType = "int" },
                            new DatabaseColumn(Table, "San+itized") { StoreType = "int" }
                        },
                        PrimaryKey = IdPrimaryKey
                    },
        ...
        public void Pluralization_of_collection_navigations()
        {
            ...
            var postTable = new DatabaseTable(Database, "Post")
            {
                ...
                {
                    IdColumn,
                    new DatabaseColumn(Table, "BlogId")
                    {
                        StoreType = "int",
                        IsNullable = true
                    }
            ...
            var dbModel = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "Table")
                    {
                        ...
                        {
                            IdColumn,
                            new DatabaseColumn(Table, "NonNullBoolWithDefault")
                            {
                                StoreType = "bit",
                                DefaultValueSql = "Default",
                                IsNullable = false
                            },
                            new DatabaseColumn(Table, "NonNullBoolWithoutDefault")
                            {
                                StoreType = "bit",
                                IsNullable = false
                            }
            ...
            var dbModel = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "Table")
                    {
                        ...
                        {
                            IdColumn,
                            new DatabaseColumn(Table, "NullBoolWithDefault")
                            {
                                StoreType = "bit",
                                DefaultValueSql = "Default",
                                IsNullable = true
        ...
        {
            var principalPkColumn = new DatabaseColumn(Table, "PrimaryKey") { StoreType = "nvarchar(450)" };
            var principalAkColumn = new DatabaseColumn(Table, "AlternateKey") { StoreType = "nvarchar(450)" };
            var principalIndexColumn = new DatabaseColumn(Table, "Index") { StoreType = "nvarchar(450)" };
            var rowversionColumn = new DatabaseColumn(Table, "Rowversion")
            {
            ...
            var dependentIdColumn = new DatabaseColumn(Table, "Id") { StoreType = "int" };
            var dependentFkColumn = new DatabaseColumn(Table, "BlogAlternateKey") { StoreType = "nvarchar(450)" };
        ...
        {
            var columnWithUnknownType = new DatabaseColumn(Table, "ColumnWithUnknownStoreType") { StoreType = "unknown_type" };
            ...
            var database = new DatabaseModel
            {
                Tables =
                {
                    new DatabaseTable(Database, "Table")
                    {
                        ...
                        {
                            IdColumn,
                            new DatabaseColumn(Table, "Column")
                            {
                                StoreType = "int",
